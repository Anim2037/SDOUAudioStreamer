...
- (void)_handleStreamer:(DOUAudioStreamer *)streamer
{
    ...
    switch ([[streamer decoder] decodeOnce]) {
        ...
        case DOUAudioDecoderWaiting:
            [streamer setStatus:DOUAudioStreamerBuffering];
            return;
        case DOUAudioDecoderEndEncounteredWithError:
            [_renderer stop];
            [streamer setDecoder:nil];
            [streamer setPlaybackItem:nil];
            [streamer setStatus:DOUAudioStreamerFinished];
...
- (void)_eventLoop
{
    ...
    while (1) {
        @autoreleasepool {
            ...
            // 文件小于decoderbufferSize时，会出现播放不出来的情况
            if (streamer != nil) {
                if ([[_currentStreamer fileProvider] expectedLength]<20280) {
                    [self _handleStreamer:streamer];
                } else if ([[_currentStreamer fileProvider] expectedLength]<_decoderBufferSize) {
                    if ([[_currentStreamer fileProvider] receivedLength]>=[[_currentStreamer fileProvider] expectedLength]) {
                        [self _handleStreamer:streamer];
                    }
                } else {
                    if ([[_currentStreamer fileProvider] receivedLength]>=_decoderBufferSize) {
                        [self _handleStreamer:streamer];
                    }
                }
            }
...
{
    pthread_setname_np("com.appwill.audio-streamer.event-loop");
