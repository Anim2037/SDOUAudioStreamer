...
- (void)dealloc
{
    ...
    if (_receivedLength<_expectedLength) {
        [fileManager removeItemAtPath:_cachedPath error:nil];
    }
...
{
    // custom code 修改保存名字
    return [[self class] cachedPathForAudioFileURL:audioFileURL];
    // custom code end
...
- (void)_requestDidReceiveResponse
{
    ...
    _cachedPath = [[self class] cachedPathForAudioFileURL:[_audioFile audioFileURL]];
...
+ (NSString *)cachedFolder
{
    NSString *cacheFolder = [NSString stringWithFormat:@"%@/AudioCaches",[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0]];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if(![fileManager fileExistsAtPath:cacheFolder]){//如果不存在,则说明是第一次运行这个程序，那么建立这个文件夹
        [fileManager createDirectoryAtPath:cacheFolder withIntermediateDirectories:YES attributes:nil error:nil];
    }
    return cacheFolder;
}

// custom code md5 产生唯一名称
+ (NSString *)cachedFileNameForKey:(NSURL *)audioFileURL
{
    const char *str = [[audioFileURL absoluteString] UTF8String];
    if (str == NULL) {
        str = "";
    }
...
{
    NSString *filename = [NSString stringWithFormat:@"%@.tmp", [self cachedFileNameForKey:audioFileURL]];
    // custom code 修改缓存目录
    NSString *cachePath = [[self class] cachedFolder];
...
- (NSString *)cachedFolder
{
    return [[self class] cachedFolder];
}
// custom code end

+ (BOOL)isCachedWithAudioFile:(id <DOUAudioFile>)audioFile
{
    NSString *url = [[audioFile audioFileURL] absoluteString];
    if (url) {
        NSString *filePath = [[self class] cachedPathForAudioFileURL:[audioFile audioFileURL]];
